...
/* ===== утиліти для числового вводу (як у CKD) ===== */
...
function cockcroftGault(age: number, sex: Sex, scrMgDl: number, weightKg: number) {
  const base = ((140 - age) * weightKg) / (72 * scrMgDl);
  return sex === 'Жіноча' ? base * 0.85 : base;
}

/* ===== інтерпретація, як у картці CKD (однакові стилі «пігулки») ===== */
...
export default function CockcroftGaultPage() {
  ...
  const canCalc = useMemo(
    () => Number.isFinite(age) && age > 0 && Number.isFinite(weight) && weight > 0 && Number.isFinite(scr) && scr > 0,
    [age, weight, scr]
  );
  ...
  function handleCalc(e: React.FormEvent) {
    e.preventDefault();
    if (!canCalc) return;
    const crcl = cockcroftGault(age, sex, scrMgDl, weight);
    setResult(Number(crcl.toFixed(1)));
  }

  async function copy() {
    if (result == null) return;
    const i = interpretGFR(result);
    const text = `Кліренс креатиніну (Cockcroft–Gault): ${result} мл/хв — ${i.stage} (${i.range}) — ${i.text}`;
    try {
  ...
  React.useEffect(() => { setResult(null); }, [sex, ageStr, weightStr, unit, scrStr]);
    ...
    <div className="min-h-screen w-full bg-gradient-to-b from-white to-slate-50">
      <div className="mx-auto max-w-3xl px-4 py-12">
        ...
        <form onSubmit={handleCalc} className="rounded-2xl bg-white/90 shadow-xl ring-1 ring-black/5 p-6 sm:p-8 space-y-6">
          ...
          <div>
            <label className="block font-semibold mb-2">Маса тіла (кг):</label>
            <input
              inputMode="decimal"
              pattern="[0-9]*[.,]?[0-9]*"
              ...
              onBlur={() => setWeightStr((v) => normalizeOnBlur(v))}
              className="w-full rounded-lg border border-slate-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          ...
          {!canCalc && (
            <p className="text-xs text-slate-500 -mt-4">Введіть вік, масу тіла та креатинін.</p>
          )}
        </form>

        {/* === ОДНАКОВА КАРТКА, ЯК У CKD === */}
        {result !== null && (() => {
          const i = interpretGFR(result);
          return (
            <div className="mt-6 rounded-xl border p-4 shadow-sm bg-white/70">
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <div className="text-2xl">
                  <span className="font-bold">Кліренс креатиніну:</span>{' '}
                  <span className="font-mono">{result}</span> мл/хв
                </div>
              ...
              <div
                className={
                  `mt-3 inline-flex items-center gap-2 rounded-full border px-3 py-1 text-sm font-semibold ${i.chip}`
                }
              >
              ...
              <p className="text-xs text-slate-500 mt-3">
                * Стадії KDIGO ґрунтуються на eGFR (мл/хв/1.73м²). Тут наведена приблизна відповідність для CrCl.
                Для класифікації ХХН використовуйте eGFR.
              </p>
            </div>
          );
        })()}
